...
class Graph(object):
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        #gradient have to be float type
        loss_node.gradient = 1.0
        graph = list(reversed(self.graph))
        for node in graph:
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        A = inputs[0]
        B = inputs[1]
        return list([np.dot(gradient, np.transpose(B)), np.dot(np.transpose(A), gradient)])
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        x = inputs[0]
        return [np.where(x > 0.0, gradient, np.where(x < 0.0, 0, np.where(x == 0.0, x, float('Inf'))))]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient*(inputs[0]-inputs[1])/inputs[0].size,-gradient*(inputs[0]-inputs[1])/inputs[0].size]
